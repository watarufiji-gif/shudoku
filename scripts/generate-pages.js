...
const { esc, escWithWbr } = require('./lib/text-wrap');
...
<body>
  ...
  <div class="book-reader-overlay" id="book-reader-overlay" role="dialog" aria-modal="true" aria-label="書評を読む">
    ...
    <div class="book-body">
      <div class="book-front-cover" id="book-cover" aria-hidden="true">
        <p class="book-front-title">${esc(title)}</p>
      </div>
...
<body>
  ...
  <!-- ヘッダー -->
  <header style="padding:calc(80px + var(--space-2xl)) 20px 40px;text-align:center;background:var(--color-bg);">
...
// esc() / escWithWbr() は scripts/lib/text-wrap.js に共通化済み（先頭でrequire）
